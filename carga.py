...
class EEUU:
    ...
    def now(self):
        ...
        history =  get_eeuu_limpio()
        with open('data/datos_eeuu_etl.json', 'r') as f: history = json.load(f)
        starttime = str(datetime.datetime.strptime(history[-1]['date'], '%Y-%m-%d')+datetime.timedelta(days=+1))

        now = 'https://earthquake.usgs.gov/fdsnws/event/1/query.geojson?starttime='+starttime+'%2000:00:00&maxlatitude=50&minlatitude=24.6&maxlongitude=-65&minlongitude=-125&minmagnitude=1&orderby=time-asc'
...
class Chile:
    ...
    def history(self):
        ...
        try:
            with open('datos_chile_etl.json', 'r') as f: history = json.load(f)
        except:
            ...
            with open('data/datos_chile_etl.json', 'r') as f: history = json.load(f)
            starttime = str(datetime.datetime.strptime(history[-1]['fecha_local'], '%Y-%m-%d')+datetime.timedelta(days=+1))[:10]

            now = 'https://earthquake.usgs.gov/fdsnws/event/1/query.geojson?starttime='+starttime+'%2000:00:00&maxlatitude=-17.5&minlatitude=-56.0&maxlongitude=-66.0&minlongitude=-81.0&minmagnitude=1&orderby=time-asc'
            now = requests.get(now).json()
            now = pd.json_normalize(now, record_path =['features'])
            now = now.to_json(orient = 'records')

            with open('data/datos_chile.json', 'w') as f: f.write(now)
    ...
    def etl(self, df):
        ...
        try:
            ...
            chile_json = json.loads(chile_json)
            chile_json = list(chile_json)
            with open('datos_chile_etl.json', 'r') as f: history = json.load(f)
            for entry in chile_json:
                if entry != history[-1]:
                    history.append(entry)
            history = json.dumps(history)

            with open('datos_chile_etl.json', 'w') as f: f.write(history)
