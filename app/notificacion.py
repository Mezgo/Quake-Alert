...
def show_notificacion():
    ...
    with col1:
        st.markdown("<h1 style='font-size: 20px;'>Notificacion Ultimo Sismo Fuerte</h1>", unsafe_allow_html=True)
    ...
    with col4:
        #st.markdown("<h1 style='font-size: 20px;'>Medidas de Prevención</h1>", unsafe_allow_html=True)
        st.empty()
    ...
    st.write("Contenedor 1")

    with col_container[0]:
        
        col_container = st.columns([0.5, 0.5])
        st.write("Contenedor 2")
        '''
        KPI
        '''
        with col_container[0]:
            #st.write("Contenedor 2 col 1")
            # KPI 1: Magnitud > 4.5
            css = """
            .card_1 {
                padding: 10px;
        ...
        col_container = st.columns([0.05,0.90,0.05])
        #st.write("Contenedor 3")
        with col_container[0]:
            folium_static(mapa)
            st.empty()
        with col_container[0]:
